...
function FiscalYearCategoryDonut({ colors, year, activeCategory, onCategoryHover }: {
  activeCategory: string | null;
  onCategoryHover: (code: string | null) => void;
  ...
  return (
    <svg viewBox="0 0 128 128" role="img" aria-label={`${year.fiscalYear}年度の横断集計カテゴリ構成比グラフ`} focusable="false">
      ...
      {segments.map(({ item: category, offsetPercentage, percentage }) => {
        ...
        return <circle onMouseEnter={() => onCategoryHover(category.code)} onMouseLeave={() => onCategoryHover(null)} opacity={activeCategory && activeCategory !== category.code ? 0.2 : 1} key={category.code} cx="64" cy="64" r={chartRadius} fill="none" stroke={colors[category.code]} strokeDasharray={`${dashLength} ${chartCircumference - dashLength}`} strokeDashoffset={dashOffset} strokeLinecap="butt" strokeWidth={activeCategory === category.code ? chartStroke + 4 : chartStroke} transform="rotate(-90 64 64)" />;
      })}
