...
function donutBackground(year: FiscalYearComparisonViewYear) {
  if (year.categoryTotal <= 0) return undefined;
  let offset = 0;
  const stops = year.categories.map((category) => {
    const start = offset;
    offset += category.percentage ?? 0;
    return `var(--fiscal-year-category-${category.code}) ${start}% ${offset}%`;
  });
...
export function FiscalYearCategoryDonuts({ amountDisplayMode, years }: {
    ...
    <section className="fiscal-year-comparison-section" aria-labelledby="fiscal-year-category-title">
      <div className="fiscal-year-comparison-section-heading">
        ...
        <div className="fiscal-year-comparison-legend" aria-label="横断集計カテゴリの凡例">
          {CROSS_AGGREGATE_CATEGORY_CODES.map((code) => <span key={code}><i className={`fiscal-year-swatch fiscal-year-swatch-${code}`} aria-hidden="true" />{CROSS_AGGREGATE_CATEGORY_LABELS[code]}</span>)}
        </div>
      </div>
      <div className="fiscal-year-category-grid" role="group" aria-label="年度別の横断集計カテゴリ構成比">
        {years.map((year) => (
          <Link key={year.fiscalYear} className="fiscal-year-category-card" to={`/?year=${year.fiscalYear}`} aria-label={`${year.fiscalYear}年度の年度ページを開く`}>
            ...
            <span className={`fiscal-year-category-donut${year.categoryTotal <= 0 ? " fiscal-year-category-donut-empty" : ""}`} style={{ background: donutBackground(year) }} aria-hidden="true">
