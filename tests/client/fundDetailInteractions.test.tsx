...
describe("Fund detail interactions", () => {
  ...
  it("can delete a planned item from the planned item list", async () => {
    const user = userEvent.setup();
    let currentFundDetail = {
      fund: { id: 1, name: "基盤研究費", awarded_amount: 5080000 },
      categories: [
    ...
    fetchMock.mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === "/api/planned-items/10" && method === "DELETE") {
        currentFundDetail = {
          ...currentFundDetail,
          plannedItems: currentFundDetail.plannedItems.filter((item) => item.id !== 10),
        };
    ...
    const fundPage = within(view.container);
    const plannedTable = await fundPage.findByRole("table", { name: "Fund planned items" });
    const plannedRow = within(plannedTable).getByText("GPU サーバ保守更新").closest(".detail-history-entry");

    expect(plannedRow).not.toBeNull();

    await user.click(within(plannedRow as HTMLElement).getByRole("button", { name: "削除" }));
  ...
  it("opens an edit modal for actual entries and saves the edited fields", async () => {
    const user = userEvent.setup();
    ...
    fetchMock.mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === "/api/actual-entries/8" && method === "PUT") {
        currentFundDetail = {
          ...currentFundDetail,
          actualEntries: [],
          plannedItems: [],
    ...
    expect(within(dialog).getByLabelText("実績日")).toHaveValue("2026/06/20");
    fireEvent.change(within(dialog).getByLabelText("実績日カレンダー"), {
      target: { value: "2026-06-25" },
    });
    expect(within(dialog).getByLabelText("実績日")).toHaveValue("2026/06/25");
    fireEvent.change(within(dialog).getByLabelText("説明"), {
      target: { value: "GPU サーバ保守 改" },
    });
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/actual-entries/8", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        fundId: 2,
    ...
    await waitFor(() => {
      expect(screen.queryByRole("dialog", { name: "精算項目を編集" })).not.toBeInTheDocument();
    });
  });
  ...
  it("can cancel a planned item from the edit modal", async () => {
    const user = userEvent.setup();
    let currentFundDetail: {
      fund: { id: number; name: string; awarded_amount: number };
      categories: Array<{
    ...
    fetchMock.mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === "/api/planned-items/10/cancel" && method === "POST") {
        currentFundDetail = {
          ...currentFundDetail,
          plannedItems: [],
        };
    ...
    const dialog = await screen.findByRole("dialog", { name: "計画項目を編集" });
    await user.click(within(dialog).getByRole("button", { name: "計画を取り消す" }));

    expect(fetchMock).toHaveBeenCalledWith("/api/planned-items/10/cancel", {
      method: "POST",
    });
    expect(await fundPage.findByText("未精算の計画項目はまだありません。")).toBeInTheDocument();
  });

  it("can delete a cancelled planned item from the planned item history", async () => {
    const user = userEvent.setup();
    let currentFundDetail = {
      fund: { id: 1, name: "基盤研究費", awarded_amount: 5080000 },
      categories: [],
    ...
    fetchMock.mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === "/api/planned-items/10" && method === "DELETE") {
        currentFundDetail = {
          ...currentFundDetail,
          plannedItemHistory: currentFundDetail.plannedItemHistory.filter((item) => item.id !== 10),
        };
    ...
    await user.click(within(cancelledRow as HTMLElement).getByRole("button", { name: "削除" }));

    expect(fetchMock).toHaveBeenCalledWith("/api/planned-items/10", {
      method: "DELETE",
    });
    await waitFor(() => {
      expect(fundPage.queryByText("取消済み研究会")).not.toBeInTheDocument();
    });
  });
